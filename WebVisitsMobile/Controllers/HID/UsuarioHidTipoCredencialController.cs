...
namespace WebVisitsMobile.Controllers.HID
{
    ...
    public class UsuarioHidTipoCredencialController : ControllerBase
    {
        private readonly IMapper _mapper;
        private readonly IUriService _uriService;
        private readonly IAccesorService _accesorService;
        private readonly IPlataformaService _plataformaService;
        private readonly IUsuarioHidTipoCredencialService _tipoCredencialService;
        public UsuarioHidTipoCredencialController(
            IMapper mapper,
            IUriService uriService,
            IAccesorService accesorService,
            IPlataformaService plataformaService,
            IUsuarioHidTipoCredencialService tipoCredencialService
            )
        {
            _mapper = mapper;
        ...
        [HttpGet]
        [ProducesResponseType((int)HttpStatusCode.OK, Type = typeof(ApiResponse<IEnumerable<UsuarioHidTipoCredencialRespDTO>>))]
        [ProducesResponseType((int)HttpStatusCode.BadRequest)]
        public async Task<IActionResult> GetAll([FromQuery] UsuarioHidTipoCredencialQueryFilter filters)
        {
            try
            {
                ...
                string strUriNextPage = _uriService.GetUserHidTypeCredentialUri(filters, Url.RouteUrl(nameof(GetAll))).ToString();

                var response = new ApiResponse<IEnumerable<UsuarioHidTipoCredencialRespDTO>>(true, "Consulta exitosa", 200, dataDTO);
        ...
        public async Task<IActionResult> GetById(Guid id)
        {
            try
            {
                ...
                var dataDTO = _mapper.Map<UsuarioHidTipoCredencialRespDTO>(data);
                var response = new ApiResponse<UsuarioHidTipoCredencialRespDTO>(true, "Consulta ejecutada", 200, dataDTO);
