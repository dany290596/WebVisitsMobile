...
namespace WebVisitsMobile.Controllers.Administracion.Modulo
{
    [Authorize]
    ...
    public class ModuloController : ControllerBase
    {
        ...
        [HttpGet]
        [ProducesResponseType((int)HttpStatusCode.OK, Type = typeof(ApiResponse<IEnumerable<ModuloRespDTO>>))]
        [ProducesResponseType((int)HttpStatusCode.BadRequest)]
        public async Task<IActionResult> GetAll([FromQuery] ModuloQueryFilter filters)
        {
            try
            {
                ...
                var data = _moduloService.GetAll(filters, empresaId);
                var dataDTO = _mapper.Map<IEnumerable<ModuloRespDTO>>(data);

                string strUriPreviousPage = _uriService.GetModulePaginationUri(filters, Url.RouteUrl(nameof(GetAll))).ToString();
                string strUriNextPage = _uriService.GetModulePaginationUri(filters, Url.RouteUrl(nameof(GetAll))).ToString();

                var response = new ApiResponse<IEnumerable<ModuloRespDTO>>(true, "Consulta exitosa", 200, dataDTO);
        ...
        public async Task<IActionResult> GetById(Guid id)
        {
            try
            {
                ...
                var data = await _moduloService.GetById(id, empresaId);
                var dataDTO = _mapper.Map<ModuloRespDTO>(data);
                var response = new ApiResponse<ModuloRespDTO>(true, "Consulta ejecutada", 200, dataDTO);
        ...
        public async Task<IActionResult> Inactivate([Required] Guid id)
        {
            ...
            var result = await _moduloService.Inactivate(id, token.UsuarioId);
            if (!result)
            {
        ...
        public async Task<IActionResult> Reactivate([Required] Guid id)
        {
            ...
            var result = await _moduloService.Reactivate(id, token.UsuarioId);
            if (!result)
            {
