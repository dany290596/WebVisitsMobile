...
namespace WebVisitsMobile.Controllers.Ubicacion
{
    ...
    public class PaisEstadoController : ControllerBase
    {
            ...
            )
        {
            ...
            _paisEstadoService = paisEstadoService;
        }

        [HttpGet]
        [ProducesResponseType((int)HttpStatusCode.OK, Type = typeof(ApiResponse<IEnumerable<PaisEstadoRespDTO>>))]
        [ProducesResponseType((int)HttpStatusCode.BadRequest)]
        public async Task<IActionResult> GetAll([FromQuery] PaisEstadoQueryFilter filters)
        {
            try
            {
                ...
                var data = await _paisEstadoService.GetAll(filters, empresaId);
                var dataDTO = _mapper.Map<IEnumerable<PaisEstadoRespDTO>>(data);

                string strUriPreviousPage = _uriService.GetStateUri(filters, Url.RouteUrl(nameof(GetAll))).ToString();
                string strUriNextPage = _uriService.GetStateUri(filters, Url.RouteUrl(nameof(GetAll))).ToString();

                var response = new ApiResponse<IEnumerable<PaisEstadoRespDTO>>(true, "Consulta exitosa.", 200, dataDTO);
        ...
        public async Task<IActionResult> GetById(Guid id)
        {
            try
            {
                ...
                var data = await _paisEstadoService.GetById(id, empresaId);
                var dataDTO = _mapper.Map<PaisEstadoRespDTO>(data);
                var response = new ApiResponse<PaisEstadoRespDTO>(true, "Consulta ejecutada", 200, dataDTO);
        ...
        public async Task<IActionResult> Inactivate([Required] Guid id)
        {
            ...
            var result = await _paisEstadoService.Inactivate(id, token.UsuarioId);
            if (!result)
            {
        ...
        public async Task<IActionResult> Reactivate([Required] Guid id)
        {
            ...
            var result = await _paisEstadoService.Reactivate(id, token.UsuarioId);
            if (!result)
            {
        ...
        [HttpPost]
        public async Task<IActionResult> Create(PaisEstadoReqDTO data)
        {
            if (!Guid.TryParse(Request.Headers["Empresa"], out var empresaId))
            {
                return BadRequest("El header de la empresa es inválido.");
            ...
            var mapper = _mapper.Map<PaisEstado>(data);

            var book = await _paisEstadoService.Create(mapper, token.UsuarioId);
            if (book == null)
            {
                return StatusCode(500, new ApiResponse<string>(false, "ocurrió un error.", 500, null));
            }

            PaisEstadoRespDTO dto = _mapper.Map<PaisEstadoRespDTO>(mapper);

            var response = new ApiResponse<PaisEstadoRespDTO>(true, "El registro se creó correctamente.", 200, dto);

            return StatusCode(200, response);
        }

        [HttpPut]
        public async Task<IActionResult> Update(Guid id, PaisEstadoReqDTO data)
        {
            if (!Guid.TryParse(Request.Headers["Empresa"], out var empresaId))
            {
                return BadRequest("El header de la empresa es inválido.");
            ...
            var mapper = _mapper.Map<PaisEstado>(data);
            mapper.Id = id;
            var result = await _paisEstadoService.Update(mapper, token.UsuarioId);
            if (!result)
            {
                return StatusCode(500, new ApiResponse<string>(false, "ocurrió un error.", 500, null));
