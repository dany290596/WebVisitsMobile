...
namespace WebVisitsMobile.Controllers.Ubicacion
{
    ...
    public class CiudadController : ControllerBase
    {
        ...
        [HttpGet]
        [ProducesResponseType((int)HttpStatusCode.OK, Type = typeof(ApiResponse<IEnumerable<CiudadRespDTO>>))]
        [ProducesResponseType((int)HttpStatusCode.BadRequest)]
        public async Task<IActionResult> GetAll([FromQuery] CiudadQueryFilter filters)
        {
            try
            {
                ...
                var data = await _ciudadService.GetAll(filters, empresaId);
                var dataDTO = _mapper.Map<IEnumerable<CiudadRespDTO>>(data);

                string strUriPreviousPage = _uriService.GetCityUri(filters, Url.RouteUrl(nameof(GetAll))).ToString();
                string strUriNextPage = _uriService.GetCityUri(filters, Url.RouteUrl(nameof(GetAll))).ToString();

                var response = new ApiResponse<IEnumerable<CiudadRespDTO>>(true, "Consulta exitosa.", 200, dataDTO);
        ...
        public async Task<IActionResult> GetById(Guid id)
        {
            try
            {
                ...
                var data = await _ciudadService.GetById(id, empresaId);
                var dataDTO = _mapper.Map<CiudadRespDTO>(data);
                var response = new ApiResponse<CiudadRespDTO>(true, "Consulta ejecutada", 200, dataDTO);
        ...
        public async Task<IActionResult> Inactivate([Required] Guid id)
        {
            ...
            var result = await _ciudadService.Inactivate(id, token.UsuarioId);
            if (!result)
            {
        ...
        public async Task<IActionResult> Reactivate([Required] Guid id)
        {
            ...
            var result = await _ciudadService.Reactivate(id, token.UsuarioId);
            if (!result)
            {
        ...
        [HttpPost]
        public async Task<IActionResult> Create(CiudadReqDTO data)
        {
            if (!Guid.TryParse(Request.Headers["Empresa"], out var empresaId))
            {
                return BadRequest("El header de la empresa es inválido.");
            ...
            var mapper = _mapper.Map<Ciudad>(data);

            var book = await _ciudadService.Create(mapper, token.UsuarioId);
            if (book == null)
            {
                return StatusCode(500, new ApiResponse<string>(false, "ocurrió un error.", 500, null));
            }

            CiudadRespDTO dto = _mapper.Map<CiudadRespDTO>(mapper);

            var response = new ApiResponse<CiudadRespDTO>(true, "El registro se creó correctamente.", 200, dto);

            return StatusCode(200, response);
        }

        [HttpPut]
        public async Task<IActionResult> Update(Guid id, CiudadReqDTO data)
        {
            if (!Guid.TryParse(Request.Headers["Empresa"], out var empresaId))
            {
                return BadRequest("El header de la empresa es inválido.");
            ...
            var mapper = _mapper.Map<Ciudad>(data);
            mapper.Id = id;
            var result = await _ciudadService.Update(mapper, token.UsuarioId);
            if (!result)
            {
                return StatusCode(500, new ApiResponse<string>(false, "ocurrió un error.", 500, null));
